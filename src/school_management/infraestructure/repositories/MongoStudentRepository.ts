...
export class MongoStudentRepository implements StudentInterface {
    ...
    async findByUUID(uuid: string): Promise<Student | null> {
        try {
            ...
            if (result) {
                let subject = new Student(result.name, result.email, result.tutor);
                return subject;
            }
            return Promise.resolve(null);
        } catch (error) {
        ...
        try {
            ...
            if (result) {
                return result.map((element: any) => {
                    let tutor = new Student(element.name, element.email, element.tutor);
    ...
    async addSubjectToStudent(uuidStudent: string, uuidSubject: string): Promise<Student | null> {
        try {
            ...
            if (result) {
                ...
                if (result) {
                    let student = new Student(result.name, result.email, result.tutor);
                    return Promise.resolve(student);
                }
